...
class _CreateArticleScreenState extends State<CreateArticleScreen> {
  ...
  Future<void> _submit() async {
    ...
    if (_selectedCategory == null) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Pilih kategori artikel!')));
      return;
    }
    if (_imageFile == null) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Unggah thumbnail wajib!')));
      return;
    }
    ...
    if (mounted) {
      ...
      if (success) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Artikel berhasil diajukan!')));
        Navigator.pop(context);
      } else {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Gagal mengajukan artikel. Pastikan Anda sudah login.')));
      }
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('Ajukan Artikel', style: TextStyle(color: Color(0xFF1B4F1E), fontWeight: FontWeight.bold)),
            ...
            children: [
                    ...
                    child: _imageFile != null
                        ...
                        : Column(
                            ...
                            children: const [
                              ...
                              SizedBox(height: 8),
                              Text('Unggah Thumbnail *', style: TextStyle(fontWeight: FontWeight.bold)),
                              Text('Format JPG/JPEG, PNG maks 2MB', style: TextStyle(fontSize: 12, color: Colors.grey)),
                            ],
                          ),
                  ),
                ),
                  ...
                  child: Column(
                    children: [
                        ...
                        child: Row(
                          children: [
                            ...
                            IconButton(
                              ...
                              onPressed: () => _insertFormat('<ul>\n  <li>', '</li>\n</ul>'),
                              tooltip: 'Bullet List',
                            ),
                            IconButton(
                              icon: const Icon(Icons.link),
                              onPressed: () => _insertFormat('<a href="URL">', '</a>'),
                              tooltip: 'Link',
                            ),
                            IconButton(
                              icon: const Icon(Icons.image),
                              onPressed: () => _insertFormat('<img src="URL" alt="', '">'),
                              tooltip: 'Image',
                            ),
                          ],
                        ),
                      ...
                      TextFormField(
                        ...
                        maxLines: 8,
                        validator: (val) => val != null && val.length < 30 ? 'Minimal 30 karakter' : null,
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    TextFormField(
                      ...
                      validator: (val) => val != null && val.isEmpty ? 'Sumber referensi tidak boleh kosong' : null,
                      decoration: InputDecoration(
              ...
              ElevatedButton(
                ...
                child: _isSubmitting 
                    ...
                    : const Text('Ajukan Artikel', style: TextStyle(color: Colors.white, fontSize: 16, fontWeight: FontWeight.bold)),
              )
            ],
          ),
        ),
